...
export async function POST(request: NextRequest) {
  ...
  // Optional temp login even when Supabase is configured (for emergency access).
  // Only enabled when explicitly requested.
  const wantsTemp = request.nextUrl.searchParams.get("temp") === "1";
  if (wantsTemp && demoAccount && input.password === demoAccount.password) {
    const token = makeTempToken(demoAccount);
    const res = NextResponse.redirect(new URL("/dashboard", request.url), { status: 303 });
    res.cookies.set(ERP_SESSION_COOKIE, token, {
      httpOnly: true,
  ...
  return NextResponse.redirect(new URL("/dashboard" as Route, request.url), { status: 303 });
}
