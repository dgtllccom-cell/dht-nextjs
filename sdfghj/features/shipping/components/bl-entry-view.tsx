...
export function BlEntryView({ context = "purchase" }: { context?: "purchase" }) {
    ...
    () =>
      ledgers
        ...
        .map((row) => ({
          ...
          keywords: [row.code, row.name, row.currency, row.current_balance].filter(Boolean).join(" ")
        })),
  ...
  function applyLedger(value: string) {
    ...
    setForm((current) => ({
      ...current,
      ledgerId: value,
      accountNumber: ledger?.code ?? current.accountNumber,
      currencyCode: ledger?.currency ?? current.currencyCode
    }));
  }
