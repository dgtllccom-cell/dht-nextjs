            ...
            <table className="min-w-[1320px] w-full text-sm">
              <thead className="sticky top-0 z-10 bg-muted/80 text-left text-muted-foreground backdrop-blur">
                <tr>
                  ...
                  <ReportTh className="text-left">Account Code</ReportTh>
                ...
                {loading ? (
                  ...
                  filteredRows.map((row) => {
                    ...
                    return (
                      <tr
                        key={row.accountId}
                        className={cn("border-t transition hover:bg-muted/30", active ? "bg-muted/40" : "")}
                      ...
                      >
                        ...
                        <ReportTd>
                          <AccountRowActionsMenu
                            ...
                            onView={() => setSelectedAccountId(row.accountId)}
                            onEdit={() => router.push(`/dashboard/accounts/setup?accountId=${row.accountId}` as Route)}
                            onOpenAccount={() => setSelectedAccountId(row.accountId)}
                            onOpenLedger={() => {
        ...
        <aside className="h-fit rounded-lg border bg-card xl:sticky xl:top-24">
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="space-y-2">
              ...
              <PreviewRow label="Account Code" value={selectedRow?.accountCode} />
