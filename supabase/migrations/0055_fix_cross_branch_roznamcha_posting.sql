-- Fix: Allow automated transfers to post to cross-branch ledgers
-- This introduces `p_bypass_ledger_scope` to `post_roznamcha_entry` so automated RPCs
-- like `post_purchase_order_payment` can bypass manual UI restrictions.
...
begin
  ...
  v_lines := jsonb_build_array(
    jsonb_build_object(
      ...
      'description', coalesce(p_narration, 'Purchase Order Debit'),
      'debit', p_amount,
      'credit', 0,
    ...
    jsonb_build_object(
      ...
      'description', coalesce(p_narration, 'Purchase Order Credit'),
      'debit', 0,
      'credit', p_amount,
      'currency', p_currency_code,
      'usdRate', v_line_rate
  ...
  v_entry_id := post_roznamcha_entry(
    ...
    p_narration,
    v_lines,
    true -- p_bypass_ledger_scope: BYPASS RLS SO CROSS-BRANCH TRANSACTIONS CAN BE AUTOMATED
  );
...
values ('0055_fix_cross_branch_roznamcha_posting', 'applied', now())
on conflict (name)
