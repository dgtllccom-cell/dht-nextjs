...
export function PurchaseTransferErpReportView({
  ...
  /* ── Transfer to Payment ────────────────────────────────────── */
  function handleTransferToPayment() {
    if (!d) return;
    const params = new URLSearchParams({
      purchaseOrderNo: d.purchaseBookingOrderNumber || "",
      supplierName: d.supplierName || "",
      supplierLedger: d.purchaseAccountNumber || "INV-001",
      payableAmount: String(totalPurchaseAmount),
      currency: d.currency || "USD",
      narration: `Payment for ${d.purchaseBookingOrderNumber} - ${d.productName || d.goodsDescription || "Purchase"}`,
      debitAccount: "AP-" + (d.purchaseBookingOrderNumber?.slice(-3) || "001"),
      creditAccount: "CASH-001",
      remainingBalance: String(remainingBalance),
      transactionDate: d.purchaseDate || new Date().toISOString().slice(0, 10),
      bookingRef: d.purchaseBookingOrderNumber || ""
    });
    router.push(`/dashboard/journal/purchase-order-payment/advance?${params.toString()}`);
  }
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <header className="sticky top-0 z-50 bg-[#0f2942] text-white px-4 py-2.5 flex items-center justify-between shadow-lg print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* ★ PRIMARY: Transfer To Payment button */}
          <Button
            type="button"
            size="sm"
            onClick={handleTransferToPayment}
            className="h-8 bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[10px] uppercase tracking-wide px-3 shadow-md gap-1.5 border-none"
          >
      ...
      <main className="mx-auto max-w-[900px] py-6 px-4 space-y-4 print:py-0 print:px-0 print:max-w-none">
        ...
        <SectionCard icon={<CreditCard className="h-4 w-4" />} title="Payment Information" badge={d.paymentStatus}>
          <div className="grid sm:grid-cols-2 gap-x-8">
            <div>
              <InfoRow label="Total Purchase Amount" value={`${money(totalPurchaseAmount)} ${currency}`} mono />
              <InfoRow label="Advance Amount (30%)" value={`${money(advanceAmount)} ${currency}`} mono />
              <InfoRow label="Remaining Balance" value={`${money(remainingBalance)} ${currency}`} mono />
            </div>
            <div>
              <InfoRow label="Payment Status" value={d.paymentStatus || "-"} />
        ...
        <SectionCard icon={<BookOpen className="h-4 w-4" />} title="Accounting / Ledger Impact">
          ...
          <div className="grid sm:grid-cols-3 gap-x-8 mb-4">
            ...
            <InfoRow label="Posting Status" value={d.form_data?.workflow?.journalStatus || "Posted"} />
          </div>
            ...
            <table className="w-full text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {glEntries.map((entry, idx) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono font-black text-[#0f2942]">{entry.glCode}</td>
                    <td className="px-4 py-3 font-semibold text-slate-700">{entry.accountName}</td>
              ...
              <tfoot>
                <tr className="bg-[#0f2942] text-white font-black text-[11px]">
                  <td colSpan={2} className="px-4 py-3 uppercase tracking-wider">Total</td>
                  <td className="px-4 py-3 text-right font-mono">{money(totalDebit)}</td>
                  <td className="px-4 py-3 text-right font-mono">{money(totalCredit)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 flex flex-col sm:flex-row items-center justify-between gap-3 print:hidden shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black text-emerald-900">Ready to process payment?</p>
              <p className="text-[10px] text-emerald-600 font-semibold mt-0.5">
                All transaction data will auto-fill in the Cash Payment screen. No re-entry needed.
              </p>
            </div>
          </div>
          <Button
            type="button"
            onClick={handleTransferToPayment}
            className="h-10 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[11px] uppercase tracking-wide px-5 shadow-md gap-2 border-none shrink-0"
          >
